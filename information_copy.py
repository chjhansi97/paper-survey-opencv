...
class ImageFormApp:
    ...
    def __init__(self, folder_path):
        ...
        #self.image_files = [f for f in os.listdir(os.path.dirname(folder_path)) if f.lower().endswith(('.jpg'))]
        ...
        def save_form():
            ...
            for text in self.labels_texts:
                if text == "120":
                    ...
                    data[text] = self.entries[text].get() or "NULL"

            # Save form data to a CSV file
            csv_filename = "manual_entry_data.csv"
            print("Saving data to:", csv_filename) 
            with open(csv_filename, "a", newline="") as csvfile:
                fieldnames = ["Survey No"] + self.labels_texts
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

                if os.stat(csv_filename).st_size == 0:
                    writer.writeheader()

                row = {"Survey No": os.path.splitext(os.path.basename(self.folder_path))[0]}
                row.update(data)
                writer.writerow(row)
                print("Data saved successfully.")  
            with open("processed_manual_entries.txt", "a") as file:
                file.write(os.path.basename(self.folder_path) + "\n")
                print("Image filename saved:", self.current_index)
    ...
    def next_image(self):
        ...
        processed_images = self.load_processed_images()
        image_files = [f for f in os.listdir(os.path.dirname(self.folder_path)) if f.lower().endswith(('.jpg'))]
        ...
        next_index = (image_files.index(os.path.basename(self.folder_path)) + 1) % len(image_files)
        next_image_path = os.path.join(os.path.dirname(self.folder_path), image_files[next_index])
        
        app = ImageFormApp(next_image_path)
        app.root.mainloop()
